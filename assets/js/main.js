const startDay = new Date("03/01/2022");
...
const getCounter = (startDay, daysBetweenSession) => {
  ...
  const daysToNextSession =
    Math.floor(startDay.getTime() / (24 * 60 * 60 * 1000)) % daysBetweenSession;

  // Get the date of the next session
  const nextSessionDate = new Date(
    new Date().setDate(toDay.getDate() + daysToNextSession)
  );
  document.querySelector(".nextSessionDate").innerHTML =
  ...
  const newMinute =
    60 - (toDay.getMinutes() === 0 ? 60 : toDay.getMinutes()) - 1;
  const newSecond = 60 - (toDay.getSeconds() === 0 ? 60 : toDay.getSeconds());

  const counter = document.querySelector(".counterTime");

  counter.innerHTML = `${daysToNextSession
    .toString()
    .padStart(2, "0")}:${newHour.toString().padStart(2, "0")}:${newMinute
    .toString()
